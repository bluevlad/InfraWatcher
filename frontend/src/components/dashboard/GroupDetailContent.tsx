...
interface GroupDetailContentProps {
  ...
  /** Drawer 임베드 시 외부 헤더에 그룹명이 있으므로 내부 타이틀 생략 가능 */
  showTitle?: boolean;
...
const GroupDetailContent: React.FC<GroupDetailContentProps> = ({
  ...
  useEffect(() => {
    ...
    const totalMemLimit = groupContainers.reduce(
      (s, c) => s + (c.memory_limit > 0 ? c.memory_limit : 0),
      0,
    );
    ...
    setSummary({
      ...
      total_memory_percent:
        totalMemLimit > 0 ? Math.round((totalMemUsage / totalMemLimit) * 10000) / 100 : 0,
    });
  }, [snapshot, groupName]);
    ...
    <div>
      {showTitle && (
        <Title level={4} style={{ marginBottom: 16 }}>
          <Tag color={color} style={{ fontSize: 16, padding: '2px 12px' }}>
            {groupName}
          </Tag>
        </Title>
      )}
            ...
            <Statistic
              ...
              suffix={
                summary?.unhealthy_count ? (
                  <span style={{ fontSize: 14, color: '#ff4d4f' }}>
                    {' '}
                    / {summary.unhealthy_count} <CloseCircleOutlined />
                  </span>
                ) : undefined
              }
            />
            ...
            <Statistic
              ...
              suffix={
                <span style={{ fontSize: 14, color: '#999' }}>
                  {' '}
                  / {(summary?.total_memory_percent ?? 0).toFixed(1)}%
                </span>
              }
      ...
      <div style={{ marginBottom: 16 }}>
        <ContainerCardGrid containers={groupContainers} onContainerClick={onContainerClick} />
      </div>
      ...
      {groupHealthChecks.length > 0 && <HealthCheckPanel healthChecks={groupHealthChecks} />}
    </div>
  );
};
